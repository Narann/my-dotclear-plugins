...
class tplSmiliesManager
{
	public static function toolbar($attr) {
		...
		foreach ($smilies as $smiley) {
			if ($smiley['onToolbar']) {
				$res .= ' <img src="'.$smiley['url'].'" alt="'.htmlspecialchars($smiley['code']).'" title="'.htmlspecialchars($smiley['code']).'" onclick="javascript:smiliesManagerInsertSmiley(\''.$attr['textarea'].'\', \''.htmlspecialchars(str_replace('\'', '\\\'', str_replace('\\', '\\\\', $smiley['code']))).'\');" style="cursor:pointer" />';
			}
		}
